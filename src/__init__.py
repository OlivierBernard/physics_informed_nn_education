...
def get_root() -> Path:
    """Resolves the root directory for the `src` package.

    Returns:
        Path to the root directory for the `src` package.
    """
    return Path(__file__).resolve().parent


def get_home() -> Path:
    """Resolves the home directory for the `src` library, used to save/cache data reusable across
    scripts/runs.

    Returns:
        Path to the home directory for the `src` library.
    """
    load_dotenv()
    home = os.getenv(ENV_HOME)
    if home is None:
        user_cache_dir = os.getenv("XDG_CACHE_HOME", DEFAULT_CACHE_DIR)
        home = os.path.join(user_cache_dir, "src")
